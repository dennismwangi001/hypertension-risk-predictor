...
def load_models():
    models = {}
    # Using lowercase keys to match your train_models.py and saved .pkl files
    ...
    for m_name in required_models:
        ...
        if os.path.exists(path):
            # Store with a nice title for display, but keep the key logic consistent
...
# Note: Ensure this matches the 'feature_names.pkl' generated by your training script
safe_features = [
...
if page == "📊 Dashboard":
    ...
    if submit_button:
        if not models:
            ...
            # Default to XGBoost, fallback to Random Forest if needed
            model = models.get('Xgboost') or models.get('Random Forest')
            
            # Create DataFrame matching the EXACT feature names expected by the model
            ...
            # CRASH PREVENTION: Ensure columns are in the exact order the model expects
            try:
            ...
            try:
                ...
                # Show predictions from all models for comparison
                results = []
